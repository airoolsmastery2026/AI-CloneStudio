...
import { Play, Activity, Clock, MoreVertical, Zap, Layers, Video, ArrowRight, Database, CheckCircle2, Loader2, AlertCircle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ lang }) => {
  ...
  const { projects, userCredits, playVideo, activeVideoId } = useStudio();
  ...
  const stats = [
    { label: lang === 'EN' ? 'Processing' : 'Đang xử lý', value: activeProjects.length.toString(), icon: Loader2, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: lang === 'EN' ? 'Credits Available' : 'Tín dụng khả dụng', value: userCredits.toString(), icon: Zap, color: 'text-yellow-400', bg: 'bg-yellow-500/10' },
    { label: lang === 'EN' ? 'Ready to Download' : 'Sẵn sàng tải', value: completedProjects.length.toString(), icon: CheckCircle2, color: 'text-green-400', bg: 'bg-green-500/10' },
  ];
      ...
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-display font-bold text-white">
            {lang === 'EN' ? 'Production Pipeline' : 'Quy trình Sản xuất'}
          </h2>
          <p className="text-gray-400 mt-1 flex items-center gap-2 text-sm">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
            {lang === 'EN' ? 'System Operational • All Engines Online' : 'Hệ thống hoạt động • Các công cụ đã sẵn sàng'}
          </p>
        </div>
        <div className="flex gap-3">
           <button 
             onClick={() => navigate('/mass-production')}
             className="flex items-center gap-2 bg-studio-800 hover:bg-studio-700 text-white px-5 py-2.5 rounded-xl font-bold transition-all border border-white/10 text-sm"
           >
             <Layers size={16} />
             {lang === 'EN' ? 'Batch View' : 'Xem theo mẻ'}
           </button>
           <button 
             onClick={() => navigate('/create')}
             className="flex items-center gap-2 bg-studio-accent hover:bg-studio-accentHover text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-indigo-500/25 text-sm"
           >
             <Zap size={16} />
             {lang === 'EN' ? 'Quick Create' : 'Tạo nhanh'}
           </button>
        </div>
      </header>
        ...
        <div className="glass-panel rounded-2xl flex flex-col h-full border-t-4 border-t-gray-500">
           <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
              <h3 className="font-bold text-white flex items-center gap-2">
                 ...
                 {lang === 'EN' ? 'Input Queue' : 'Hàng chờ đầu vào'}
              </h3>
           ...
           <div className="p-4 space-y-3 flex-1 overflow-y-auto max-h-[500px] hide-scrollbar">
              {queuedProjects.length === 0 ? (
                 <div className="text-center py-10 text-gray-500 text-sm italic">
                    {lang === 'EN' ? 'No items in queue.' : 'Hàng chờ trống.'}
                 </div>
              ) : (
                    ...
                    <div key={p.id} className="bg-studio-900/50 p-3 rounded-xl border border-white/5 hover:border-white/20 transition-all cursor-pointer" onClick={() => navigate(`/analysis/${p.id}`)}>
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-black rounded-lg overflow-hidden shrink-0">
                             <img src={p.thumbnail} className="w-full h-full object-cover opacity-70" />
                          </div>
        ...
        <div className="glass-panel rounded-2xl flex flex-col h-full border-t-4 border-t-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.1)]">
           <div className="p-4 border-b border-white/5 flex justify-between items-center bg-blue-500/10">
              <h3 className="font-bold text-white flex items-center gap-2">
                 ...
                 {lang === 'EN' ? 'Processing Engine' : 'Đang xử lý'}
              </h3>
           ...
           <div className="p-4 space-y-3 flex-1 overflow-y-auto max-h-[500px] hide-scrollbar">
              {activeProjects.length === 0 ? (
                 <div className="text-center py-10 text-gray-500 text-sm italic">
                    {lang === 'EN' ? 'Engine idle.' : 'Máy nhàn rỗi.'}
                 </div>
              ) : (
                       ...
                       <div className="flex items-center gap-3 relative z-10">
                          ...
                          <div className="min-w-0 flex-1">
                             ...
                             <div className="flex justify-between items-center mt-1">
                                <span className="text-xs text-blue-300">{p.status}</span>
        ...
        <div className="glass-panel rounded-2xl flex flex-col h-full border-t-4 border-t-green-500">
           <div className="p-4 border-b border-white/5 flex justify-between items-center bg-white/5">
              <h3 className="font-bold text-white flex items-center gap-2">
                 ...
                 {lang === 'EN' ? 'Completed Output' : 'Hoàn thành'}
              </h3>
           ...
           <div className="p-4 space-y-3 flex-1 overflow-y-auto max-h-[500px] hide-scrollbar">
              {completedProjects.length === 0 ? (
                 <div className="text-center py-10 text-gray-500 text-sm italic">
                    {lang === 'EN' ? 'No completed videos.' : 'Chưa có video nào.'}
                 </div>
              ) : (
                 completedProjects.map(p => (
                    <div 
                      key={p.id} 
                      className="bg-studio-900/50 p-3 rounded-xl border border-white/5 hover:border-green-500/30 transition-all cursor-pointer group"
                      onClick={() => playVideo(p.id)}
                    >
                       <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                             <h4 className="text-sm font-bold text-white truncate">{p.title}</h4>
                             <p className="text-xs text-green-400 mt-0.5">{lang === 'EN' ? 'Ready to download' : 'Sẵn sàng tải xuống'}</p>
